...
F_0_1_Values1 = np.array(
    [[1, 1], [0, 0], [2, 2], [1, 1], [4, 4]]
)  # Define some arbitrary inflow data for each year and element
F_1_2_Values1 = np.array([[1, 1], [1, 1], [32, 32], [1, 1], [4, 4]])  #
F_2_1_Values1 = np.array(
    [[0, 0], [1, 1], [30, 30], [0, 0], [0, 0]]
)  # Define flow back to process 1, make sure mass balance holds.
...
Model_Aspects = [
    ...
    "Element",
    "Unity",
]  # aspects of the model to be considered. Must be the same as the items of the ModelClassification_Test1 dictionary
# Define a simple index table with the three aspects time, element, and unity, using the modelclassification above.
IndexTable = pd.DataFrame(
    {
        "Aspect": Model_Aspects,  # 'Time' and 'Element' must be present!
        "Description": ["Time", "Element", "Unity"],
        "Dimension": [
            "Time",
            "Element",
            "Unity",
        ],  # must be the same as 'Dimenstion' in the classification above
        "Classification": [
            ModelClassification_Test1[Aspect] for Aspect in Model_Aspects
        ],
        "IndexLetter": ["t", "e", "i"],
    }
)  # Unique one letter (upper or lower case) indices to be used later for calculations. t for time, e for element

IndexTable.set_index(
    "Aspect", inplace=True
...
class KnownResultsTestCase(unittest.TestCase):
    ...
    def test_Dimensions_MyMFA_Test1(self):
        ...
        np.testing.assert_array_equal(
            MyMFA_Test1.FlowDict["F_1_2"].Values.shape[0], MyMFA_Test1_Flowshape1[0]
        )
        np.testing.assert_array_equal(
            MyMFA_Test1.FlowDict["F_1_2"].Values.shape[1], MyMFA_Test1_Flowshape1[1]
        )
